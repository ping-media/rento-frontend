...
const SoldOutCard = ({ BookingEndDate, MaintenanceEndDate }) => {
  ...
  const availabilityText = useMemo(() => {
    if (BookingEndDate) return "Booked";
    if (MaintenanceEndDate) return "Temporarily Unavailable";
    return "Temporarily Unavailable";
  }, [BookingEndDate, MaintenanceEndDate]);
  ...
  return (
    <div className="absolute w-full h-full bg-white bg-opacity-40 z-10 lg:z-5 rounded-b-lg">
      ...
      <p
        className="flex items-center justify-center px-1 lg:px-2 py-1 h-[12%] lg:h-[10%] bg-theme-black text-gray-100 text-xs lg:text-sm truncate"
      ...
      >
        ...
        <span className="font-semibold uppercase tracking-wide md:tracking-wider">
          {availabilityText}
        </span>
      </p>
    </div>
